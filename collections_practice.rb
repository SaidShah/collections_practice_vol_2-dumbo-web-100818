...
given_array.each do |values|
  values.each { |looking_for, value_needed| cool << values if looking_for == :temperature && value_needed =="cool"}
  end
  cool
end
...
def organize_schools(given_array)
 ...
 sorted_locations = {}
  ...
  given_array.each { |school, location| sorted_locations[location.values[0]] << school }
  locations
end
